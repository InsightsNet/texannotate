    ...
    with open(response[0], "r") as f:
        ...
        if isinstance(data, dict) and data["type"] == "file":
            try:
                ...
                #print(os.path.join(dir_out, data["name"]))
                # bring the cursor to the beginning, erase the current line, and dont make a new line
        ...
        for file in data:
            ...
            if flatten:
                path = os.path.join(dir_out, os.path.basename(file_path))
            else:
                path = file_path#os.path.join(dir_out, file_path)
